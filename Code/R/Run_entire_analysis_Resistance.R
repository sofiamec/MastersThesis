...
if (onTerra==F){
  ...
  groupSize<-c(3,5)#,30,50)                                            # m = Number of samples in each group (total nr samples = 2*m)
  sequencingDepth<-c(10000,100000)#,10000,1000000,5000000,10000000)      # d = Desired sequencing depth per sample
  sequencingDepthName<-c("10k","100k")# "10K","500k","1M", "5M", "10M")         # dD = Displayed names for sequencing depths
}
