...
sequencingDepth<-c(100000,500000)     # d,  Gut2: 5000000, Marine: 10000000
sequencingDepthName<-c("100k","500k")      # dD, Gut2: 5M, Marine: 10M
...
for (group in 1:length(groupSize)){ # looping over m
  ...
  for (seq in 1:length(sequencingDepth)) { # looping over d
    ...
    if(savePlot == TRUE){
      path_save <-  sprintf("../../Result/%s/IntermediatePlots/individualROCs_rep%s.pdf", saveName, saveExpDesign, repeats)
    ...
    if(savePlot == TRUE){
      path_save <-  sprintf("../../Result/%s/IntermediatePlots/meanROC_%s_rep%s.pdf", saveName, saveExpDesign, repeats)
...
# Save tables:
write.csv(meanAUCfinal, file=sprintf("../../Result/%s/AUC_10q%d_rep%s.csv", saveName,10*q, repeats))
...
print(heatmapAUCtot)
if(savePlot == TRUE){
  path_save <-  sprintf("../../Result/%s/heatmap_AUCtot_10q%d_rep%s.pdf", saveName,q*10, repeats)
...
print(heatmapAUC5)
if(savePlot == TRUE){
  path_save <-  sprintf("../../Result/%s/heatmap_AUC5_10q_rep%d_rep%s.pdf", saveName,10*q, repeats)
...
print(heatmapAUC10)
if(savePlot == TRUE){
  path_save <-  sprintf("../../Result/%s/heatmap_AUC10_10q%d_rep%s.pdf", saveName,q*10,repeats)
...
print(heatmapTPR5)
if(savePlot == TRUE){
  path_save <-  sprintf("../../Result/%s/heatmap_TPR5_10q%d_rep%s.pdf", saveName,q*10,repeats)
...
print(heatmapTPR10)
if(savePlot == TRUE){
  path_save <-  sprintf("../../Result/%s/heatmap_TPR10_10q%d_rep%s.pdf", saveName,q*10, repeats)
...
for (group in 1:length(groupSize)){
  ...
  if(savePlot == TRUE){
    path_save <-  sprintf("../../Result/%s/meanROC_10q%d,groupsize_%d_rep%s.pdf", saveName,10*q, M, repeats)
...
for (seq in 1:length(sequencingDepth)) {
  ...
  if(savePlot == TRUE){
    path_save <-  sprintf("../../Result/%s/meanROC_10q%d_depth_%d_rep%s.pdf", saveName,10*q,D,repeats)
