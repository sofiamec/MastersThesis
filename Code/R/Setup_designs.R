# This setup script assigns correct names for a given design as well as preprocesses the selected dataset

####################################################################################################################################

# Real settings (Full-scale setup)
if (onTerra==T){
...
# Extra-settings (Used for analysing trade-off)
...
if (runStrata==T){
  ...
  strataClass<-factor(c("low","medium","high"), levels=c("low","medium","high"))    # Should correspond to the number of stratas
}
...
if(saveName == "Gut2"){
  ...
  Gut2 <- remove_low_counts(Gut2Intermediate)                        # Fiters out genes with low counts
  Data = Gut2
  ...
  Marine <- remove_low_counts(MarineIntermediate)                           # Fiters out genes with low counts
  Data = Marine
  ...
  Resistance <- remove_low_counts(ResistanceIntermediate)                           # Fiters out genes with low counts
  
  Data = Resistance
    
  runStrata=F # The strata analysis cannot be run for this dataset due to too few genes                                          
  relations<-c(3000000,5000000)
  if (onTerra==T){
    sequencingDepth<-c(10000,100000,500000,1000000,5000000,10000000)
    sequencingDepthName<-c("10 k","100 k","500 k","1 M", "5 M", "10 M")
...
#===================================================================================================================================
## Setting up the right environment and naming designs

# Save-names for each design
...
# Displayed name in plots for each design
...
for (effect in 1:length(effectsizes)) {           # looping over q
  ...
  # Creating the main experimental designs
  ...
  # Creating 3 extra designs for trade-off analysis                               
  if (extraDesigns==T){
